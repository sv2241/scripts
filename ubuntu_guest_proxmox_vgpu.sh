#!/bin/bash
#Source: https://github.com/kamilllooo/Proxmox/tree/main/scripts
...
apt update && apt install git sysfsutils dkms -y
...
sed -i 's/^GRUB_CMDLINE_LINUX_DEFAULT=".*"/GRUB_CMDLINE_LINUX_DEFAULT="intel_iommu=on iommu=pt i915.enable_guc=3"/' /etc/default/grub
